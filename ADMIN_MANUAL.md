...
### 管理画面を開く
ブラウザで以下のファイルを開きます（またはサーバー上のURL）。
`schedule_admin.html`

### 更新手順
1. **新規スケジュールの追加**:
    - **DATE**: 日付を `YYYY.MM.DD` 形式で入力します（例: `2026.02.11`）。
    - **TYPE**: スケジュールの種類を選択します（休館日、イベント等）。
    - **TITLE**: タイトルを入力します（例: 建国記念の日）。
    - **Details**: 詳細を入力します。
    - **[ADD SCHEDULE]** ボタンを押すとリストに追加されます。
2. **編集・削除**:
    - リストの **[EDIT]** ボタンで編集、**[DELETE]** ボタンで削除できます。
3. **ファイルを更新**:
    - **[DOWNLOAD JS]** ボタンを押し、`schedule_data.js` をダウンロードします。
    - ダウンロードしたファイルを、プロジェクトの `js` フォルダ内の `schedule_data.js` に上書き保存します。
4. **公開**: GitHubに変更をプッシュします。
